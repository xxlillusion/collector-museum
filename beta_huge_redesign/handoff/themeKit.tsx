 ...
 *   SERIF → t.fontDisplay (headings) / t.fontBody (copy) · SANS → t.fontBody
 *   PAGE_BG → t.pageBg · museumHoverCss → t.hoverCss
...
export type ThemeId = 'refined' | 'night' | 'lobby';
...
const refined: Theme = (() => {
  ...
  return {
    id: 'refined', label: 'Museum Refined',
    ...
    radius: 2,
    primaryButton: { background: GOLD, color: '#1a1614', border: 'none', padding: '13px 34px', fontSize: 13, letterSpacing: '0.16em', fontFamily: SERIF, cursor: 'pointer', borderRadius: 2 },
...
const night: Theme = (() => {
  ...
  return {
    id: 'night', label: 'Show Floor · Night',
    bg: BG, pageBg: BG,
    ...
    primaryButton: { background: RED, color: BG, border: 'none', padding: '14px 28px', fontFamily: BODY, fontWeight: 700, fontSize: 13, letterSpacing: '0.06em', cursor: 'pointer', borderRadius: 3, boxShadow: `4px 4px 0 ${CREAM}` },
...
const lobby: Theme = (() => {
  ...
  return {
    ...
    radius: 2,
    primaryButton: { background: OX, color: IVORY, border: 'none', padding: '13px 28px', fontFamily: MONO, fontWeight: 600, fontSize: 12, letterSpacing: '0.12em', cursor: 'pointer', borderRadius: 2 },
...
export const THEMES: Record<ThemeId, Theme> = { refined, night, lobby };
...
function readStoredTheme(): ThemeId {
  try {
    ...
    if (v === 'refined' || v === 'night' || v === 'lobby') return v;
  } catch { /* private mode */ }
  return 'refined';
}
